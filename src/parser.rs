...
pub fn into_hashmap(lines: &Vec<String>) -> HashMap<String, String> {
...
fn parse_line_into_hashmap(line: &String, map: &mut HashMap<String, String>) {
...
fn is_identifier_line(line: &String) -> bool {
    line.chars().nth(0).unwrap() == DV_ID_LINE_PREFIX
}

fn parse_identifier_line_into_hashmap(line: &String, map: &mut HashMap<String, String>) {
    let caps = REGEXP_IDENTIFIER_LINE.captures(line).unwrap();
...
fn parse_single_value_line_into_hashmap(line: &String, map: &mut HashMap<String, String>) {
    let caps = REGEXP_SINGLE_VALUE_LINE.captures(line).unwrap();
